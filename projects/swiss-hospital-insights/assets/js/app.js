...
if (languageSwitcher) {
  const languageButtons = Array.from(
    languageSwitcher.querySelectorAll(".lang-btn"),
  );
  if (languageButtons.length) {
    ...
    const initialActive =
      languageSwitcher.querySelector(".lang-btn.active") || languageButtons[0];
    if (initialActive) {
...
const scrollHideElements = document.querySelectorAll(
  ".header-left, .swiss-cross",
);
...
if (procedureSelector && catalogToggle && catalogPanels) {
  const toggleLabel = catalogToggle.querySelector(
    ".finder-catalog-toggle-label",
  );
  const collapsedLabel =
    catalogToggle.dataset.labelCollapsed || "Browse the full catalogue";
  const expandedLabel =
    catalogToggle.dataset.labelExpanded || "Hide the full catalogue";

  const setCatalogExpanded = (expanded) => {
    const isExpanded =
      !procedureSelector.classList.contains("catalog-collapsed");
...
const hospitalDisplayNameOverrides = {
  ...
  "Spital Thurgau AG Kantonsspitäler Frauenfeld & Münsterlingen":
    "Spital Thurgau – KSp Frauenfeld & Münsterlingen",
  "Hôpital du Valais Centre hospitalier du Valais Romand CHVR":
    "Hôpital du Valais – CHVR (Valais Romand)",
  "Etablissements Hospitaliers du Nord Vaudois (eHnv)":
    "eHnv – Etablissements du Nord Vaudois",
  "Groupement Hospitalier de l'Ouest Lémanique (GHOL) SA":
    "GHOL – Ouest Lémanique",
  "Center da Sanda Engiadina Bassa Ospidal d'Engiadina Bassa":
    "CSEB – Ospidal d’Engiadina Bassa",
  "Pôle santé Vallée de Joux Hôpital de la Vallée de Joux":
    "Hôpital de la Vallée (Vallée de Joux)",
  "Swiss Medical Network Hospitals SA Clinica Sant'Anna":
    "Swiss Medical Network – Clinica Sant’Anna",
  "Gruppo ospedaliero Moncucco Clinica Santa Chiara SA":
    "Gruppo Moncucco – Clinica Santa Chiara",
  "Swiss Medical Network Hospitals SA Clinica Ars Medica":
    "Swiss Medical Network – Clinica Ars Medica",
  "Swiss Medical Network Hospitals SA Clinique Générale Ste-Anne":
    "Swiss Medical Network – Clinique Générale Ste-Anne",
  "Swiss Medical Network Hospitals SA Clinique de Valère":
    "Swiss Medical Network – Clinique de Valère",
  "Swiss Medical Network Hospitals SA Privatklinik Obach":
    "Swiss Medical Network – Privatklinik Obach",
  "Swiss Medical Network Hospitals SA Privatklinik Siloah":
    "Swiss Medical Network – Privatklinik Siloah",
  "Swiss Medical Network Hospitals SA Privatklinik Villa im Park":
    "Swiss Medical Network – Privatklinik Villa im Park",
  "Swiss Medical Network Hospitals SA Privatklinik Belair":
    "Swiss Medical Network – Privatklinik Belair",
};

const getHospitalDisplayName = (name) =>
  hospitalDisplayNameOverrides[name] ?? name;
...
function inferHospitalMeta(
  name,
  coordinatesMap = hospitalDatasetCache.coordinates,
) {
...
function buildHospitalDataset(entries, coordinatesMap) {
  ...
  entries.forEach((rawEntry) => {
    ...
    const institution =
      typeof rawEntry.institution === "string"
...
function parsePopulationDataset(csvText) {
  ...
  for (const rawLine of lines) {
    ...
    if (
      line.startsWith(
        "Structure of the permanent resident population by canton,",
      )
    ) {
...
if (finderRoot) {
  ...
  const activeLocale = SUPPORTED_LOCALES.includes(pageLocale)
    ? pageLocale
    : "en";
  ...
  const translations = {
    en: {
      ...
      messages: {
        ...
        selectProcedureNational:
          "Choose a procedure to display national case totals.",
        ...
        noProceduresMatch:
          "No procedures match your search. Try a different keyword.",
        ...
        hospitalsPerformingPlural:
          "{count} hospitals performing this procedure",
        ...
        cantonNoHospitals:
          "No hospitals in canton {canton} match the current selection.",
        cantonSummary:
        ...
        cantonComparisonPrompt:
          "Select a canton to compare against the national average.",
        selectProcedureComparison:
          "Choose a procedure to display the canton comparison.",
        cantonComparisonNoData:
          "Not enough data to calculate rates for this canton.",
        cantonComparisonTitle: "Case frequency per 100k residents",
        cantonComparisonLead:
          "Cases per 100k residents: {cantonRate} in {canton} vs {nationalRate} nationwide.",
        ...
        hospitalDetail: {
          ...
          note: "Rates are shown as published in CH-IQI reporting and may be suppressed when case volumes are low.",
          infoAddress: "Address",
          ...
          proceduresDescription:
            "CH-IQI tracks {count} specialized procedures in this hospital ({total} total cases in 2023).",
          proceduresCode: "Code",
          proceduresName: "Procedure",
          proceduresCases: "Cases 2023",
          proceduresShare: "Share of F volume",
          proceduresEmpty:
            "No F-coded procedures were reported for this hospital.",
    ...
    de: {
      ...
      messages: {
        ...
        chooseProcedure:
          "Wählen Sie eine Behandlung, um die Spitalliste zu sehen.",
        selectProcedureNational:
          "Wählen Sie eine Behandlung, um die nationalen Fallzahlen anzuzeigen.",
        selectProcedureMap:
          "Wählen Sie eine Behandlung, um die Spitalkarte zu laden.",
        selectProcedureCantonal:
          "Wählen Sie eine Behandlung, um kantonale Details zu sehen.",
        ...
        noProceduresMatch:
          "Keine Behandlungen passen zur Suche. Versuchen Sie einen anderen Begriff.",
        tryAdjustFilters:
          "Versuchen Sie einen anderen Eingriff oder passen Sie die Filter an.",
        ...
        hospitalsPerformingPlural:
          "{count} Spitäler führen diesen Eingriff durch",
        ...
        cantonSelectPrompt:
          "Wählen Sie einen Kanton, um lokale Spitaldetails zu sehen.",
        cantonNoHospitals:
          "Im Kanton {canton} passt kein Spital zur aktuellen Auswahl.",
        cantonSummary:
        ...
        cantonComparisonPrompt:
          "Wählen Sie einen Kanton, um ihn mit dem nationalen Durchschnitt zu vergleichen.",
        selectProcedureComparison:
          "Wählen Sie eine Behandlung, um den Kantonsvergleich anzuzeigen.",
        cantonComparisonNoData:
          "Für diesen Kanton können keine Raten berechnet werden.",
        cantonComparisonTitle: "Fallhäufigkeit pro 100 000 Einwohner",
        cantonComparisonLead:
          "Fälle pro 100 000 Einwohner: {cantonRate} in {canton} vs {nationalRate} schweizweit.",
        cantonComparisonHigher:
        ...
        cantonComparisonEven:
          "{canton} entspricht dem nationalen Durchschnitt.",
        ...
        quickPicksDescription:
          "Springen Sie direkt zu einer häufig nachgefragten CH-IQI-Behandlung.",
        hospitalDetail: {
          title: "Spitaldetails",
          ...
          note: "Die Kennzahlen entsprechen den CH-IQI-Veröffentlichungen und können bei kleinen Fallzahlen unterdrückt werden.",
          infoAddress: "Adresse",
          infoLocation: "Ort",
          ...
          proceduresDescription:
            "Der CH-IQI erfasst in diesem Spital {count} spezialisierte Leistungen (insgesamt {total} Fälle 2023).",
          proceduresCode: "Code",
    ...
    fr: {
      ...
      messages: {
        ...
        chooseProcedure:
          "Sélectionnez une intervention pour afficher la liste des hôpitaux.",
        selectProcedureNational:
          "Sélectionnez une intervention pour afficher les totaux nationaux.",
        selectProcedureMap:
          "Sélectionnez une intervention pour charger la carte des hôpitaux.",
        selectProcedureCantonal:
          "Sélectionnez une intervention pour voir les détails cantonaux.",
        loadingData: "Chargement des données…",
        loadingMap: "Chargement de la carte…",
        failedToLoad: "Échec du chargement des données.",
        datasetError:
          "Impossible de charger l’ensemble de données hospitalier.",
        noHospitalsFilters: "Aucun hôpital ne correspond aux filtres actuels.",
        noHospitalVolumes:
          "Aucune donnée de cas n’est disponible pour cette sélection.",
        noHospitalsSearch: "Aucun hôpital ne correspond à votre recherche.",
        noProceduresMatch:
          "Aucune intervention ne correspond à votre recherche. Essayez un autre mot-clé.",
        tryAdjustFilters:
          "Essayez une autre intervention ou ajustez les filtres.",
        ...
        hospitalsPerformingPlural:
          "{count} hôpitaux réalisent cette intervention",
        ...
        cantonSelectPrompt:
          "Sélectionnez un canton pour voir les détails locaux.",
        cantonNoHospitals:
          "Aucun hôpital du canton {canton} ne correspond à cette sélection.",
        cantonSummary:
        ...
        cantonComparisonPrompt:
          "Sélectionnez un canton pour le comparer à la moyenne nationale.",
        selectProcedureComparison:
          "Choisissez une intervention pour afficher la comparaison cantonale.",
        cantonComparisonNoData:
          "Impossible de calculer un taux pour ce canton.",
        cantonComparisonTitle: "Fréquence des cas pour 100 000 habitants",
        cantonComparisonLead:
          "Cas pour 100 000 habitants : {cantonRate} dans {canton} contre {nationalRate} au niveau suisse.",
        cantonComparisonHigher:
        ...
        mapNoData:
          "Aucune donnée cartographique disponible pour cette sélection.",
        ...
        quickPicksDescription:
          "Accédez directement à une intervention CH-IQI très consultée.",
        hospitalDetail: {
          ...
          note: "Les valeurs proviennent des publications CH-IQI et peuvent être masquées lorsque les volumes sont faibles.",
          infoAddress: "Adresse",
          ...
          proceduresDescription:
            "Le CH-IQI recense {count} interventions spécialisées dans cet hôpital (total {total} cas en 2023).",
          proceduresCode: "Code",
    ...
    it: {
      ...
      messages: {
        ...
        chooseProcedure:
          "Seleziona un intervento per visualizzare l’elenco degli ospedali.",
        selectProcedureNational:
          "Seleziona un intervento per mostrare i totali nazionali.",
        selectProcedureMap:
          "Seleziona un intervento per caricare la mappa degli ospedali.",
        selectProcedureCantonal:
          "Seleziona un intervento per vedere i dettagli cantonali.",
        ...
        noHospitalVolumes:
          "Non sono disponibili dati di casi per questa selezione.",
        noHospitalsSearch: "Nessun ospedale corrisponde alla ricerca.",
        noProceduresMatch:
          "Nessun intervento corrisponde alla ricerca. Prova con un’altra parola chiave.",
        ...
        cantonSelectPrompt:
          "Seleziona un cantone per vedere i dettagli locali.",
        cantonNoHospitals:
          "Nel cantone {canton} nessun ospedale corrisponde a questa selezione.",
        cantonSummary:
        ...
        cantonComparisonPrompt:
          "Seleziona un cantone per confrontarlo con la media nazionale.",
        selectProcedureComparison:
          "Scegli una procedura per mostrare il confronto cantonale.",
        cantonComparisonNoData:
          "Non è possibile calcolare il tasso per questo cantone.",
        cantonComparisonTitle: "Frequenza dei casi per 100 000 abitanti",
        cantonComparisonLead:
          "Casi per 100 000 abitanti: {cantonRate} in {canton} rispetto a {nationalRate} a livello svizzero.",
        cantonComparisonHigher:
        ...
        quickPicksDescription:
          "Vai subito a un intervento CH-IQI molto consultato.",
        hospitalDetail: {
          ...
          note: "I valori corrispondono alle pubblicazioni CH-IQI e possono essere nascosti con bassi volumi di casi.",
          ...
          proceduresDescription:
            "Il CH-IQI rileva {count} interventi specialistici in questo ospedale (totale {total} casi nel 2023).",
  ...
  const translate = (path, replacements = {}) => {
    const template =
      resolvePath(localeTranslations, path) ??
      resolvePath(defaultTranslations, path) ??
      path;
  ...
  const FALLBACK_PROCEDURE_SCHEMA = [
    {
      id: "cardiology",
      procedures: ["A.3.1.F", "A.4.1.F", "A.5.1.F", "A.7.2.F", "A.7.3.F"],
    },
    {
      id: "neurosciences",
      procedures: ["B.2.3.F", "B.3.1.F", "B.4.1.F", "Z.4.5.F"],
    },
    {
      id: "oncology",
      procedures: ["D.3.1.F", "E.4.11.F", "G.4.1.F", "K.1.1.F", "Z.4.42.F"],
    },
    { id: "urology", procedures: ["H.2.1.F", "H.3.1.F", "H.3.2.F", "H.5.1.F"] },
    {
      id: "transplantation",
      procedures: ["L.5.1.F", "Z.4.33.F", "Z.4.34.F", "Z.4.35.F", "Z.4.36.F"],
    },
    {
      id: "musculoskeletal",
      procedures: ["Z.4.37.F", "Z.4.38.F", "Z.4.39.F", "Z.4.40.F"],
    },
  ];
    ...
    if (!procedureTranslationCache.promise) {
      procedureTranslationCache.promise = fetch(
        ...
        .then((response) => {
          if (!response.ok) {
            throw new Error(
              `Failed to load procedure descriptions (${response.status})`,
            );
          }
          return response.text();
        })
  ...
  const buildProcedureCatalog = (entries) => {
    if (entries && entries.length) {
      const collator = new Intl.Collator(
        activeLocale === "en" ? "en" : `${activeLocale}-CH`,
        {
          sensitivity: "base",
        },
      );
      const sortedCodes = entries
        ...
        .sort((a, b) =>
          collator.compare(getProcedureName(a), getProcedureName(b)),
        );
      ...
      return Array.from(groupedByLetter.entries())
        ...
        .map(([letter, procedures]) => {
          ...
          const fallbackName =
            procedures.find((proc) => typeof proc?.name === "string")?.name ??
            "";
          ...
          if (exampleName) {
            ...
            label =
              ...
              translatedLabel !== "messages.letterCategoryLabel"
                ? translatedLabel
                : `${letter} - ${exampleName}`;
          }
          return {
  ...
  const cantonOptionMap = new Map(
    cantonOptions.map((option) => [option.value, option]),
  );
  ...
  function initializeFinderUi(procedureCatalog) {
    const finderProcedureSearch = document.getElementById(
      "finder-procedure-search",
    );
    const finderCategoryTabs = document.getElementById("finder-category-tabs");
    const finderProcedureList = document.getElementById(
      "finder-procedure-list",
    );
    ...
    const finderCantonSummary = document.getElementById(
      "finder-canton-summary",
    );
    const finderCantonList = document.getElementById("finder-canton-list");
    const finderCantonComparisonCard = document.getElementById(
      "finder-canton-comparison-card",
    );
    const finderCantonComparisonCaption = document.getElementById(
      "finder-canton-comparison-caption",
    );
    const finderCantonComparisonChart = document.getElementById(
      "finder-canton-comparison-chart",
    );
    const finderCantonFlag = document.getElementById("finder-canton-flag");
    const finderCantonFlagImage = finderCantonFlag
      ? finderCantonFlag.querySelector("img")
      : null;
    ...
    const finderQuickDescription = document.getElementById(
      "finder-quick-description",
    );
    ...
    const detailMessage = (key, replacements = {}) => {
      ...
      if (
        ...
        translation !== `messages.${messageKey}`
      ) {
        return translation;
      }
    ...
    const hospitalDetail = (() => {
      ...
      const buildContent = (entry, context = {}) => {
        ...
        const subtitleRaw = procedureLabel
          ? detailMessage("subtitle", { procedure: procedureLabel })
          : "";
        const subtitle =
          subtitleRaw && subtitleRaw !== "Procedure: "
            ? subtitleRaw
            : subtitleRaw;
        ...
        const summaryItems = [
          {
            label: detailMessage("cases2023"),
            value: formatInteger(entry.cases),
          },
        ...
        if (
          typeof switzerlandLabel !== "string" ||
          switzerlandLabel === "kpi.switzerland"
        ) {
        ...
        const websiteUrl =
          typeof recordInfo.website === "string"
            ? recordInfo.website.trim()
            : "";
        const mapUrl =
      ...
      const enhanceContent = () => {
        ...
        const getDefaultDirection = (key) => {
          ...
          return (
            header?.dataset.defaultDirection ||
            (key === "cases" || key === "share" ? "desc" : "asc")
          );
        };
        ...
        headers.forEach((header) => {
          header.addEventListener("click", () => {
            ...
            if (currentSort.key === key) {
              ...
              direction =
                header.dataset.defaultDirection || getDefaultDirection(key);
            }
    ...
    const setQuickPickCopy = () => {
      ...
      if (finderQuickDescription) {
        ...
        finderQuickDescription.textContent =
          ...
          description !== "messages.quickPicksDescription"
            ? description
            : "Jump straight to a CH-IQI procedure people look up most often.";
      }
      if (finderQuickList) {
    ...
    const updateQuickPickState = () => {
      const activeCode = state.hasUserSelection
        ? state.selectedProc?.code
        : null;
    ...
    const setupQuickPicks = () => {
      ...
      const picks = QUICK_PICK_CODES.map((code) =>
        findProcedureEntry(code),
      ).filter(Boolean);
      if (!picks.length) {
      ...
      finderQuickList.innerHTML = picks
        .map(({ procedure }) => {
          ...
          const displayName =
            quickPickLabels?.[procedure.code] || procedure.name;
    ...
    function getResultsScrollAnchor() {
      if (finderRoot) {
        const explicitAnchor = finderRoot.querySelector(
          "[data-finder-results-anchor]",
        );
        if (explicitAnchor) {
      ...
      cantonDropdownOptions.forEach((optionEl, index) => {
        ...
        if (isSelected) {
          ...
          if (!cantonDropdownOpen && cantonDropdownMenu) {
            cantonDropdownMenu.setAttribute(
    ...
    function focusCantonDropdownOption(index) {
      ...
      const clampedIndex = Math.max(
        0,
        Math.min(index, cantonDropdownOptions.length - 1),
      );
    ...
    function closeCantonDropdown() {
      ...
      cantonDropdownOptions.forEach((optionEl) =>
        optionEl.classList.remove("is-focused"),
      );
    ...
    function ensureCantonDropdown() {
      ...
      cantonDropdownToggle.addEventListener("keydown", (event) => {
        if (event.key === "ArrowDown" || event.key === "ArrowUp") {
          ...
          if (event.key === "ArrowDown") {
            ...
            focusCantonDropdownOption(
              selectedIndex >= 0 ? selectedIndex : lastIndex,
            );
          }
      ...
      cantonDropdownMenu.addEventListener("click", (event) => {
        const optionEl = event.target.closest(
          ".finder-canton-dropdown__option",
        );
        if (optionEl) {
          event.preventDefault();
    ...
    function renderProcedureControls() {
      ...
      let activeCategory =
        procedureCatalog.find((cat) => cat.id === state.selectedCategory) ??
        null;
      ...
      if (
        ...
        activeCategory.procedures.length
      ) {
        const hasSelected = activeCategory.procedures.some(
      ...
      const categoriesWithMatches = procedureCatalog.map((category) => ({
        ...category,
        hasMatch: category.procedures.some((proc) =>
          matchesProcedure(proc, query),
        ),
      }));
      ...
      finderCategoryTabs.innerHTML = categoriesWithMatches
        .map((category) => {
          const isActive =
            !isSearching && category.id === state.selectedCategory;
      ...
      finderCategoryTabs.querySelectorAll("button").forEach((btn) => {
        btn.addEventListener("click", () => {
          const category = procedureCatalog.find(
            (cat) => cat.id === btn.dataset.category,
          );
          if (!category) {
            return;
          }
      ...
      const hasProcedures = groupsToRender.some(
        (group) => group.procedures && group.procedures.length,
      );
      ...
      if (!hasProcedures) {
        ...
        if (state.shouldScrollToProcedures) {
          ...
          const target =
            finderProcedureList.closest(".finder-procedure-panel") ??
            finderProcedureList;
          if (target) {
            requestAnimationFrame(() => {
          ...
          const options = group.procedures
            .map((proc) => {
              const isActive =
                ...
                state.selectedProc?.code === proc.code;
              return `
      ...
      finderProcedureList
        .querySelectorAll(".finder-procedure-option")
        .forEach((btn) => {
          btn.addEventListener("click", () => {
            const { code, category } = btn.dataset;
            const categoryEntry =
              procedureCatalog.find((cat) => cat.id === category) ||
              procedureCatalog.find((cat) =>
                cat.procedures.some((proc) => proc.code === code),
              );
            if (!categoryEntry) {
              return;
            }
            const selected = categoryEntry.procedures.find(
              (proc) => proc.code === code,
            );
            if (!selected) {
              return;
      ...
      finderTypeToggle.querySelectorAll("button").forEach((btn) => {
        btn.addEventListener("click", () => {
          ...
          state.typeFilter[key] = !(
            state.typeFilter[key] !== false && state.typeFilter[key]
          );
    ...
    function computeAggregation(procCode) {
      ...
      const enrichedAll = entries.map((entry) => {
        const meta =
          finderDataset.meta.get(entry.institution) ||
          inferHospitalMeta(entry.institution);
      ...
      const cantonTotals = hasCantonSelection
        ? (() => {
            const cantonTotalCases = cantonHosp.reduce(
              (sum, h) => sum + h.cases,
              0,
            );
            ...
            return {
              ...
              uniShare: cantonTotalCases
                ? cantonUniCases / cantonTotalCases
                : 0,
            };
          })()
        : null;
    ...
    function renderKpis(agg) {
      ...
      const createValuesBlock = (
        ...
        const primaryLabel =
          typeof options.primaryLabel === "string" &&
        ...
        if (hasCantonSelection && secondaryValue != null) {
          ...
          const secondaryLabel =
            typeof options.secondaryLabel === "string" &&
      ...
      const applySwitzerlandLabel = (preferredLabel) => {
        ...
        const isThreePlusOne = isTwoRows && counts[0] === 3 && counts[1] === 1;

        const fallbackLabel =
          typeof preferredLabel === "string" && preferredLabel.trim()
            ? preferredLabel.trim()
            : "Switzerland";
        ...
        finderKpis
          .querySelectorAll(
            '.finder-kpi-value-label[data-label-type="switzerland"]',
          )
          .forEach((labelEl) => {
      ...
      const cantonPopulation = hasCantonSelection
        ? REGION_POPULATION[state.selectedCanton]
        : null;
    ...
    function renderTopList(agg) {
      ...
      const listLocationLabel =
        state.selectedCanton === ALL_CANTONS_OPTION
          ? msg("topHospitals")
          : msg("topHospitalsIn", {
              canton: getCantonLabel(state.selectedCanton),
            });
      ...
      const filteredBySearch = agg.hospitals.filter((h) => {
        ...
        return (
          normalizedDisplay.includes(searchLower) ||
          normalizedOriginal.includes(searchLower)
        );
      });
      ...
      if (!filteredByCanton.length) {
        finderListMeta.textContent = msg("cantonNoHospitals", {
          canton: getCantonLabel(state.selectedCanton),
        });
        finderList.innerHTML = `<p class="finder-empty">${msg("tryAdjustFilters")}</p>`;
        return;
      ...
      const totalPages = Math.max(
        1,
        Math.ceil(filteredByCanton.length / PAGE_SIZE),
      );
    ...
    `;
      ...
      finderListMeta.querySelectorAll(".finder-page-btn").forEach((btn) => {
        ...
        btn.addEventListener("click", () => {
          ...
          state.listPage = Math.min(
            Math.max(0, state.listPage + direction),
            totalPages - 1,
          );
          renderTopList(agg);
        });
      ...
      rows.forEach((row, rowIndex) => {
        ...
        const ariaLabel = detailMessage("openAria", {
    ...
    function renderMap(agg) {
      ...
      const hospitals =
        state.selectedCanton === ALL_CANTONS_OPTION
          ? hospitalsWithCoords
          : hospitalsWithCoords.filter(
              (h) => h.canton === state.selectedCanton,
            );

      const referenceHospitals = hospitals.length
        ? hospitals
        : hospitalsWithCoords;
      ...
      const bounds = {
        latMin: Math.max(
          SWITZERLAND_BOUNDS.latMin,
          targetBounds.latMin - latPadding,
        ),
        latMax: Math.min(
          SWITZERLAND_BOUNDS.latMax,
          targetBounds.latMax + latPadding,
        ),
        lonMin: Math.max(
          SWITZERLAND_BOUNDS.lonMin,
          targetBounds.lonMin - lonPadding,
        ),
        lonMax: Math.min(
          SWITZERLAND_BOUNDS.lonMax,
          targetBounds.lonMax + lonPadding,
        ),
      };
    ...
    function renderCantonDetails(agg) {
      ...
      const hasCantonSelection = state.selectedCanton !== ALL_CANTONS_OPTION;
      const cantonOption = hasCantonSelection
        ? getCantonOptionByValue(state.selectedCanton)
        : null;
      const cantonLabel = hasCantonSelection
        ? getCantonLabel(state.selectedCanton)
        : "";
      ...
      if (!leader) {
        ...
        const cantonShare = totalCanton
          ? Math.round((leader.cases / totalCanton) * 100)
          : 0;
        const nationalShare = agg.total
          ? ((leader.cases / agg.total) * 100).toFixed(1)
          : "0.0";
    ...
    function renderCantonComparison(agg) {
      ...
      const setEmptyState = (captionMessage) => {
        finderCantonComparisonCard.classList.add(
          "finder-comparison-card--empty",
        );
      ...
      const toRate = (cases, population) => {
        ...
        const casesValue = Number(cases);
        const safeCases =
          Number.isFinite(casesValue) && casesValue > 0 ? casesValue : 0;
      ...
      finderCantonComparisonCard.classList.remove(
        "finder-comparison-card--empty",
      );
      ...
      const tickMarkup = tickValues
        .map((tickValue) => {
          ...
          const tickClass =
            tickValue === 0 ? " finder-comparison-tick--zero" : "";
          return `
    ...
    function render() {
      ...
      const selectedProcedure = state.hasUserSelection
        ? state.selectedProc
        : null;
